...
|---|---|---|---|---|---|
| 0a | **🚀 FERRET LAUNCH MONETIZATION** (`dir-009`, NEW 2026-06-01) | 🔴 P0 first-dollar | queued | Ferret.com is now **105 pages, launch-ready** (built overnight). FIRE THIS FIRST. Add buy-boxes to commercial-intent pages: **DIET cluster (13 pages, ZERO buy-boxes now)** is priority — kibble, treats, hydration, food-label/transition guides; also care-supply pages (carriers, bedding, nail-clippers, litter). ReviewCard/BuyBox, `ctaHref="/go/<vendor>/<sku>?s=<slug>"` (NEVER bare URLs), real SKUs, `<AffiliateDisclosure>` above fold. ferret allow-list: Amazon, Chewy, Marshall, Wysong, Carniwhole. ALSO fix the 2 pre-existing bare Marshall links (health/aging-ferret-care, care/exercise-and-enrichment). **§1.5.b: NO buy-box on supplements-and-vitamins or any clinical/medicated product** — physical supplies only. Verify `node scripts/ci/affiliate-link-integrity.mjs` → 0 untracked + trust-guard PASS. Push own branch, PR. | this session |
