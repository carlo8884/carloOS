...
|---|---|---|---|---|---|
| 0 | **Dog.com full monetization** (156 pages, ZERO buy-boxes) | 🔴 P0 biggest-gap | queued | Flagship, most traffic, zero earning surface. Buy-boxes on ALL review/comparison/breed-gear/nutrition pages. dog-com allow-list per bot-coordination §5. §1.5.b: no Rx flea/tick or CBD buy-boxes (those were de-monetized — keep them /find-a-vet). Also fix dog-com bare-Amazon links in best-slow-feeder-bowls (dir-019). | this session |
| 0a | **PetFood.com full monetization** (102 pages, ZERO) | 🔴 P0 | queued | Nutrition site, high commercial intent. Buy-boxes on food/comparison/brand pages via /go. §1.5.b clean. | this session |
| 0b | **Fish / Lizard / Vets full monetization** | 🔴 HIGH | queued | fish (7→full: equipment/heaters/filters/food), lizard (4→full: UVB/enclosures/thermostats — high AOV), vets (1→full: editorial insurance + non-clinical supplies only, NO Rx). One branch each. | this week |
| 0c | **Ferret remaining + verify** | 🟡 | queued | Ferret diet cluster done (18pg, merged). Extend to care-supply + behavior pages. Note: CSRO already added ferret-com to brand-tag mapping + de-monetized clinical health cards (insulinoma/aging/ear-cleaner) — do NOT re-add those. | this week |
| 0d | **Horses / Saddle (after content hits 100)** | 🟡 | queued | These are still being built to 100 by CSRO content agents. Monetize once their PRs land. Resolve the schneiders/schneider slug mismatch (dir-019). | this week |
| 0b | **🚧 PR #265 + #266 fix-backs** (IR #1 + #3) | 🔴 P0 BLOCKS MERGE | queued | Two CSRO-verified blocks before #265/#266 merge. Brief: `ops/handoffs/2026-06-01-csro-to-monetization-pr265-266-fixbacks.md`. **(A)** DELETE the chlorhexidine ReviewCard+CTA on `apps/horses-com/src/app/guides/equine-dental-care/page.tsx` (QC §1.5.b — no monetized clinical products; keep the brush card). **(B)** add `horses-com` to AMAZON_BRAND+CHEWY_BRAND in `scripts/set-affiliate-tag.sh` (brand clicks untagged). Verify affiliate-link-integrity + trust-guard. | this session |
