/**
 * Racehorse care topic — /racehorse-care/[slug]
 * Long-form educational explainer with key-facts table + FAQ structured data.
 * Trust-safe: general welfare/husbandry education, no wagering advice, no
 * individualized veterinary or medical advice (defers to vet/farrier).
 */
...
import {
  allRacehorseCareSlugs,
  getRacehorseCareTopic,
} from '../../../data/racing/racehorse-care';

export function generateStaticParams() {
  return allRacehorseCareSlugs().map((slug) => ({ slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const t = getRacehorseCareTopic(params.slug);
  if (!t) {
    return buildMetadata({
      siteId: 'horse-racing',
      title: 'Topic not found',
      description: 'This racehorse care topic could not be found.',
      path: `/racehorse-care/${params.slug}`,
    });
  }
  return buildMetadata({
    siteId: 'horse-racing',
    title: `${t.title} — Racehorse Care Explained`,
    description: t.short,
    path: `/racehorse-care/${t.slug}`,
    type: 'article',
  });
}

export default function RacehorseCarePage({ params }: { params: { slug: string } }) {
  const t = getRacehorseCareTopic(params.slug);
  if (!t) notFound();

  const related = (t.related ?? [])
    .map((slug) => getRacehorseCareTopic(slug))
    ...
    <>
      ...
      <Breadcrumb
        ...
        items={[
          ...
          { name: 'Racehorse Care', href: '/racehorse-care' },
          { name: t.title },
        ]}
      />
