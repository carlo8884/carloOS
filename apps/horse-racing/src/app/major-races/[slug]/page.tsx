/**
 * Major race — /major-races/[slug]
 * Heritage/educational explainer with key-facts table + FAQ structured data.
 * Trust-safe: factual description of a public event; no wagering, no marks misuse.
 */
...
import { allMajorRaceSlugs, getMajorRace } from '../../../data/racing/major-races';

export function generateStaticParams() {
  return allMajorRaceSlugs().map((slug) => ({ slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const r = getMajorRace(params.slug);
  if (!r) {
    return buildMetadata({
      siteId: 'horse-racing',
      title: 'Race not found',
      description: 'This race could not be found.',
      path: `/major-races/${params.slug}`,
    });
  }
  return buildMetadata({
    siteId: 'horse-racing',
    title: `${r.name} — Explained`,
    description: r.short,
    path: `/major-races/${r.slug}`,
    type: 'article',
  });
}

export default function MajorRacePage({ params }: { params: { slug: string } }) {
  const r = getMajorRace(params.slug);
  if (!r) notFound();

  const related = (r.related ?? [])
    .map((slug) => getMajorRace(slug))
    ...
    <>
      ...
      <Breadcrumb
        ...
        items={[
          ...
          { name: 'Major Races', href: '/major-races' },
          { name: r.name },
        ]}
      />
      ...
      <div className="mx-auto max-w-2xl px-4 py-10">
        <article>
          ...
          {related.length > 0 && (
            <section className="mt-10 border-t border-brand-border pt-6">
              <h2 className="font-display text-lg font-bold text-brand-text-dark">Related races</h2>
              <div className="mt-3 space-y-2">
                {related.map((x) => (
                  <Link
                    key={x.slug}
                    href={`/major-races/${x.slug}`}
