/**
 * Dog.com — Breed-specific HEALTH deep-dive (shared renderer).
 *
 * One renderer, many breed health pages. Rendered by:
 *   - apps/dog-com/src/app/breeds/<static-breed>/health/page.tsx (static-folder breeds)
 *   - apps/dog-com/src/app/breeds/[slug]/health/page.tsx (template breeds)
 *
 * Static-folder breeds (EXISTING_STATIC_BREED_SLUGS) MUST have their own
 * /breeds/<breed>/health/page.tsx because Next.js static segments take
 * precedence over [slug] — a dynamic child of a static segment is never
 * reached. The static shells also let the metadata-policy gate read a literal,
 * unique title/description. This mirrors the breed × insurance architecture.
 *
 * Trust-bar (QC §1):
 ...
 *
 * All content is sourced from the vetted src/data/breed-health.ts records — this
 * renderer fabricates nothing; it presents existing data.
 */
...
export function BreedHealthContent({ slug }: { slug: string }) {
  ...
  const combined = combineSchemas(articleSchema, medicalSchema, faqSchema)
