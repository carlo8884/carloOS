...
/**
 * "Is This a Dog Emergency?" triage helper -- /tools/is-this-a-dog-emergency
 *
 * Client component. The owner selects the sign(s) they are seeing; the tool
 * returns a CONSERVATIVE urgency verdict built entirely from the existing
 * guide copy (imported from data/dog-symptom-signs). It is a triage AID,
 ...
 *   - ONLY monitor-eligible signs (09/14) -> MONITOR CLOSELY (neutral) +
...
function deriveVerdict(selected: Set<string>): Verdict | null {
  ...
  // MONITOR only if EVERY selected sign is monitor-eligible (09/14).
  ...
  if (allMonitorEligible) {
    return {
      ...
      lead: 'For a single mild episode in an otherwise normal dog, watching closely for a short window can be reasonable. Call your vet — or your nearest emergency clinic — if it persists, worsens, repeats, or you are unsure. When in doubt, seek care now.',
    }
  }
      ...
      <div className="lg:sticky lg:top-6" aria-live="polite" aria-atomic="true">
        ...
        <div className="mt-4 rounded-lg border border-brand-border bg-brand-surface p-4">
          <p className="text-xs text-brand-text-light leading-relaxed m-0">
            ...
            decide how urgently to seek care. It does not diagnose your dog — only a veterinarian can. If you
